...
function scan(input: string): ScanResult {
    ...
    for (let i = 0; i < input.length; i++) {
        if (!okayChars.test(input.charAt(i))) {
            throw new Error(
                `Illegal character '${input.charAt(i)}' at position ${i + 1}`,
            );
        }
    }
    ...
    while (i <= input.length) {
        ...
        if (curr === EOF_TOKEN) {
            ...
            if (variable) {
                ...
                tokens.push(
                    makeVariableToken(variable, i, i + variable.length),
                );
                i += variable.length;
            } else if (operator) {
                ...
                throw new Error(
                    `The character '${input.charAt(i)} shouldn't appear in position ${i}.`,
                );
            }
        }
    }
...
function readOperator(str: string, index: number): string {
    ...
    if (index < str.length - 6) {
        ...
        if (sevenChars === "implies" || sevenChars === "nequals")
            return sevenChars;
    }
    ...
    if (index < str.length - 3) {
        ...
        if (
            ["true", "nand", "niff", "neql", "!<=>", "!<->"].includes(fourChars)
        )
            return fourChars;
    }
...
function transformVariableSet(preliminary: PreliniaryScanResult): ScanResult {
    ...
    for (let j = 0; j < preliminary.tokens.length; j++) {
        if (preliminary.tokens[j].type === "variable") {
            preliminary.tokens[j].index = preliminary.variableSet[
...
export function parse(input: string): ParserResult {
    ...
    for (const currToken of tokens) {
        if (needOperand) {
            if (isOperand(currToken)) {
                ...
                throw new Error(
                    "Missing a variable, constant or open parenthesis.",
                );
            }
        } else {
            if (isBinaryOperator(currToken) || currToken.type === EOF_TOKEN) {
                ...
                while (true) {
                    ...
                    if (currOp.type === "!")
                        throw new Error("Useless negation operator.");
                    const rhs = operands.pop();
                    const lhs = operands.pop();
                    addOperand(
                ...
                throw new Error(
                    "Missing closing parenthesis or binary operator.",
                );
            }
        }
    }
    ...
    if (operators.pop().type !== EOF_TOKEN)
        throw new Error("Stack top is not EOF (logic error in parser?)");

    if (operators.length !== 0) {
        const mismatchedOp = operators.pop();
        if (mismatchedOp.type !== "(")
            throw new Error(
...
function wrapOperand(token: Token): Node {
    ...
    if (token.type === "variable")
        return new VariableNode(token.index as number);
    throw new Error(
        `Unreachable code error: Token ${token.type} isn't an operand.`,
    );
}
...
function priorityOf(token: Token): number {
    ...
    throw new Error(
        `Unreachable code error: Should never need the priority of ${token.type}`,
    );
}
...
function createOperatorNode(lhs: Node, token: Token, rhs: Node): Node {
    ...
    throw new Error(
        `Unreachable code error: Should never create an operator node from ${token.type}`,
    );
}
...
export function generateTruthTable(parseResult: ParserResult): TruthTable {
    ...
    do {
        results.push({
            ...
            result: parseResult.ast.evaluate(assignment),
        });
